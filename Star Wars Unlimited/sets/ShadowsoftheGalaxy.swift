...
struct ShadowsoftheGalaxy: View {
    ...
    private var checkedCards: [Card] {
        api.cards.filter { $0.set == "SHD" && checkedCardsManager.isChecked($0.id) }
    ...
    var body: some View {
        ScrollView {
            if checkedCards.isEmpty {
                VStack(spacing: 20) {
                    ...
                    Text("Cochez des cartes SHD pour les voir ici")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                    Spacer()
                }
        ...
        .navigationTitle("Les Ombres de la Galaxie")
