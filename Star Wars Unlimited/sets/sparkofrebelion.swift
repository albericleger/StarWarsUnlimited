...
struct sparkofrebelion: View {
    ...
    private var checkedCards: [Card] {
        api.cards.filter { $0.set == "SOR" && checkedCardsManager.isChecked($0.id) }
    ...
    var body: some View {
        ScrollView {
            if checkedCards.isEmpty {
                VStack(spacing: 20) {
                    ...
                    Text("Cochez des cartes SOR pour les voir ici")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                    Spacer()
                }
        ...
        .navigationTitle("Étincelle de la Rébellion")
