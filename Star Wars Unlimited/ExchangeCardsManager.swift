...
class ExchangeCardsManager: ObservableObject {
    @Published var cardEntries: [String: CardEntry] = [:]

    private let userDefaultsKey = "exchangeCardEntries"
    ...
    func loadExchangeCards() {
        guard let data = UserDefaults.standard.data(forKey: userDefaultsKey) else {
            print("📁 Aucune carte d'échange sauvegardée")
            return
        }
        ...
        do {
            ...
            cardEntries = entries
            print("✅ Chargé \(cardEntries.count) cartes d'échange avec styles depuis JSON")
        } catch {
            print("❌ Erreur lors du chargement des échanges: \(error.localizedDescription)")
        }
    }
    ...
    func saveExchangeCards() {
        do {
            ...
            print("💾 Sauvegardé \(cardEntries.count) cartes d'échange avec styles en JSON")
        } catch {
            print("❌ Erreur lors de la sauvegarde des échanges: \(error.localizedDescription)")
        }
    }
    ...
    func exportToJSON() -> String? {
        do {
            ...
            print("❌ Erreur lors de l'export JSON des échanges: \(error.localizedDescription)")
            return nil
        }
    }
    ...
    func importFromJSON(_ jsonString: String) {
        ...
        do {
            ...
            cardEntries = entries
            saveExchangeCards()
            print("✅ Importé \(cardEntries.count) cartes d'échange depuis JSON")
        } catch {
            print("❌ Erreur lors de l'import JSON des échanges: \(error.localizedDescription)")
        }
    }
}
